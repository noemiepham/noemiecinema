...
const AllFilms = () => {
  ...
  const [SLists, setSelectedLists] = useState("now_playing");
  ...
  const tabsValue = [
    ...
    { id: 3, text: "Language", menu: [{ name: "en-US" }, { name: "fr-FR" }] },
    {
      id: 4,
      text: "Any rating",
  ...
  useEffect(() => {
    const fetchData = async () => {
      let url = `https://api.themoviedb.org/3/movie/now_playing?language=${SLanguages}&page=${page}`;
      console.log("url", url);
      try {
                    ...
                    <Dropdown>
                      ...
                      <Dropdown.Menu>
                        {item.menu.map((element, i) => (
                          <Dropdown.Item
                            key={element + i}
                            onClick={() => setSLanguages(element.name)}
                          >
                            {element.name}
                          </Dropdown.Item>
                        ))}
